...
export default function ProfilePage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || isImageUploading) return;
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE_URL}/profile/image`, {
        method: 'PUT',
        body: formData,
      });
  ...
  const handleLikeToggle = async (postId: string) => {
    ...
    setPosts((prev) =>
      prev.map((p) =>
        p.id === postId ? { ...p, isLikedByMe: newLiked, likeCount: newCount } : p
      )
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPosts((prev) =>
          prev.map((p) =>
            p.id === postId ? { ...p, isLikedByMe: data.liked, likeCount: data.likeCount } : p
          )
        );
      } else {
        // 롤백
        setPosts((prev) =>
          prev.map((p) =>
            p.id === postId ? { ...p, isLikedByMe: post.isLikedByMe, likeCount: post.likeCount } : p
          )
        );
      }
    } catch {
  ...
  const getFollowButtonText = () => {
    if (!profile) return '';
    if (profile.isFollowedByMe) return '언팔로우';
    if (profile.isFollowingMe) return '맞팔로우';
    return '팔로우';
  };
        ...
        <div className="flex items-center px-4 py-6">
          ...
          <div
            ...
            className={`w-20 h-20 rounded-full flex-shrink-0 flex items-center justify-center overflow-hidden ${
              isMyProfile ? 'cursor-pointer hover:opacity-80' : ''
            } ${!profile.profileImageUrl ? 'bg-gradient-to-r from-instagram-purple to-instagram-primary' : ''}`}
          >
        ...
        {!isMyProfile && (
          <div className="px-4 pb-4">
            <button
              ...
              className={`w-full py-2 rounded-lg text-sm font-semibold transition-colors ${getFollowButtonStyle()}`}
            >
